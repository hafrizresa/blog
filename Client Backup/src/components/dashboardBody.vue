...
<div class="row pt-4 ">
    ...
    <div class="col-md-8">
        ...
        <p> {{element.body}}</p> 
        <p class="text-muted">Author <a href="#"> {{element.userId.name}} </a></p>
    </div>
</div>
    <div class="row">
    <router-link :to="`console/edit-article/${element._id}`"><button class="btn" style="margin-right:20px"> Edit</button></router-link>
  ...
  methods: {
    deleteArticle(data) {
      if (confirm("are you sure want delete this article?")) {
        axios
          .delete("http://localhost:3000/article", {
            data: { id: data, token: localStorage.getItem("token") }
          })
          .then(data => {
            return axios.post("http://localhost:3000/article/user", {
